...
class WindForecastController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        foreach ($stationObs->dataset->location ?? [] as $loc) {
            ...
            if (is_array($county)) {
                foreach ($county as $c) {
                    if (!array_key_exists($c, $defaultLocation[$this->countyToTaiwan[$c]])) {
                        $county = $c;
                        break;
                    }
                }
            ...
            if (is_array($county) || array_key_exists($county, $defaultLocation[$this->countyToTaiwan[$county]]))
                continue;

            $defaultLocation[$this->countyToTaiwan[$county]][$county] = [
        ...
        foreach ($windForecast->WindForecastInformation->AreaForecastData ?? [] as $areaForecastData) {
            ...
            if(((string) $areaForecastData->WindData->attributes()['type']) == 'average'){
                $data[$key]['location'][$this->countyToTaiwan[(string) $areaForecastData->attributes()['area']]][(string) $areaForecastData->attributes()['area']]['wind'] =  (string) $areaForecastData->WindData->Level;
            } else {
                $data[$key]['location'][$this->countyToTaiwan[(string) $areaForecastData->attributes()['area']]][(string) $areaForecastData->attributes()['area']]['gust'] =  (string) $areaForecastData->WindData->Level;
            }
        }
