...
class RainfallForecastController extends Controller
{
    ...
    private function format(string $path, $alert_value): array
    {
        ...
        $data = ['alert_value' => (int)$alert_value,'location' =>  ['n' => [], 'm' => [], 's' => [], 'y' => [], 'h' => [], 'e' => []]];
        foreach ($rainfallForecast->PrecipitationInformation->AreaForecastData ?? [] as $areaForecastData) {
            if(((string) $areaForecastData->Precipitation->attributes()['region']) == 'flat'){
                $data['location'][Transformer::parseRainfallFcstCounty((string) $areaForecastData->attributes()['area'])][(string) $areaForecastData->attributes()['area']]['flat'] =  (string) $areaForecastData->Precipitation->Value;
            } else {
                $data['location'][Transformer::parseRainfallFcstCounty((string) $areaForecastData->attributes()['area'])][(string) $areaForecastData->attributes()['area']]['mountain'] =  (string) $areaForecastData->Precipitation->Value;
            }
        }
